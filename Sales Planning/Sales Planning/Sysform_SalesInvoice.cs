        ...
        public static void processItemEventbefore(SAPbouiCOM.Form oForm, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                if (pVal.EventType == SAPbouiCOM.BoEventTypes.et_FORM_LOAD)
                {
                    ...
                    SAPbouiCOM.Item oCancelButton = oForm.Items.Item("2");
                    SAPbouiCOM.Item oItem1 = oForm.Items.Add("btnJE", SAPbouiCOM.BoFormItemTypes.it_BUTTON);

                    oItem1.Top = oCancelButton.Top;
                    oItem1.Left = oCancelButton.Left + 70;
                    oItem1.Width = oCancelButton.Width + 50;
                    oItem1.Height = oCancelButton.Height;
                    oItem1.Enabled = true;

                    oButton = (SAPbouiCOM.Button)oItem1.Specific;
                    oButton.Caption = "Create Journal Entry";
                    ...
                    case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED:
                        if (pVal.ItemUID == "btnJE")
                        {
                            ...
                            if (DocNum != null)
                            {
                                ...
                                if (rsFindJournalEntry.RecordCount > 0)
                                {
                                    SAP.SBOApplication.MessageBox("Journal Entry has already been created", 1, "OK", "", "");
                                    return;
                                }
                                else
                                {
                                    string queryDocEntry = $"SELECT DocEntry FROM OINV WHERE DocNum = '{DocNum.Value}'";
                                    SAPbobsCOM.Recordset rsDocEntry = (SAPbobsCOM.Recordset)SAP.SBOCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                                    rsDocEntry.DoQuery(queryDocEntry);

                                    if (rsDocEntry.RecordCount > 0)
                                    {
                                        string DocEntry = rsDocEntry.Fields.Item("DocEntry").Value.ToString();
